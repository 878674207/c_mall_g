...
public class FileUploadUtils {
    ...
    public static String uploadMinio(MultipartFile file) throws IOException {
        try {
            return uploadMinio(MinioConfig.getBucketName(), file, MimeTypeUtils.DEFAULT_ALLOWED_EXTENSION);
        } catch (Exception e) {
            throw new IOException(e.getMessage(), e);
        }
    }
     ...
     * @throws InvalidExtensionException            文件校验异常
     */
    private static String uploadMinio(String bucketName, MultipartFile file, String[] allowedExtension)
            throws FileSizeLimitExceededException, IOException, FileNameLengthLimitExceededException,
            InvalidExtensionException {
        int fileNamelength = Objects.requireNonNull(file.getOriginalFilename()).length();
        if (fileNamelength > FileUploadUtils.DEFAULT_FILE_NAME_LENGTH) {
        ...
        assertAllowed(file, allowedExtension);
        try {
            return MinioInjectUtil.uploadFile(bucketName, file);
        } catch (Exception e) {
            throw new IOException(e.getMessage(), e);
        }
    }
    ...
    public static void downloadFile(String fileUrl, HttpServletRequest request, HttpServletResponse response) {
        ...
        MinioInjectUtil.downloadFile(fileUrl, request, response);
    }
    ...
    public static String getPreviewFileUrl(String fileUrl) {
        ...
        return MinioInjectUtil.getPreviewFileUrl(fileUrl);
    }
