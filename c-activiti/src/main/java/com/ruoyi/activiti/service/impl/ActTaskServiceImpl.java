...
public class ActTaskServiceImpl implements IActTaskService {
    ...
    public Page<ActTaskDTO> selectProcessDefinitionList(PageDomain pageDomain) {
        ...
        // if (totalItems != 0) {
        // Set<String> processInstanceIdIds =
        // tasks.parallelStream().map(t -> t.getProcessInstanceId()).collect(Collectors.toSet());
        // List<ProcessInstance> processInstanceList =
        // runtimeService.createProcessInstanceQuery().processInstanceIds(processInstanceIdIds).list();
        ...
        // 获取到当前用户角色，并查询角色下的代办任务
        // List<SysRole> roles = SecurityUtils.getLoginUser().getUser().getRoles();
        // String roleKey = roles.get(0).getRoleKey();
        // TaskQuery taskQuery = taskService.createTaskQuery().taskCandidateGroup(roleKey);

        int total = taskQuery.active().list().size();
        int start = (pageDomain.getPageNum() - 1) * pageDomain.getPageSize();
        int pageSize = pageDomain.getPageSize();
        List<org.activiti.engine.task.Task> taskList =
            taskQuery.active().orderByTaskCreateTime().desc().listPage(start, pageSize);
        if (total != 0) {
            Set<String> processInstanceIdIds =
                taskList.parallelStream().map(t -> t.getProcessInstanceId()).collect(Collectors.toSet());
            List<ProcessInstance> processInstanceList =
                runtimeService.createProcessInstanceQuery().processInstanceIds(processInstanceIdIds).list();
            List<ActTaskDTO> actTaskDTOS = taskList.stream()
                .map(t -> new ActTaskDTO(t, processInstanceList.parallelStream()
                    .filter(pi -> t.getProcessInstanceId().equals(pi.getId())).findAny().get()))
                .collect(Collectors.toList());
            list.addAll(actTaskDTOS);
    ...
    public List<String> formDataShow(String taskID) {
        ...
        UserTask userTask =
            (UserTask)repositoryService.getBpmnModel(task.getProcessDefinitionId()).getFlowElement(task.getFormKey());
    ...
    public int formDataSave(String taskID, List<ActWorkflowFormDataDTO> awfs) throws ParseException {
        ...
        for (ActWorkflowFormDataDTO awf : awfs) {
            ActWorkflowFormData actWorkflowFormData =
                new ActWorkflowFormData(processInstance.getBusinessKey(), awf, task);
